...
class _UpcomingShiftsState extends State<UpcomingShifts> {
  ...
  void initState() {
    ...
    if(/*Provider.of<UserProvider>(context,listen: false).userData.trust['system_type']['name'] == "nhsp_api" &&*/ shiftsProvider.upComingWeeks.isNotEmpty){
      _isLoading = false;
    }else{
  ...
  Widget build(BuildContext context) {
    ...
    final ShiftsStage stage = Provider.of<ShiftsProvider>(context).upcomingShiftsStage;

    return stage == ShiftsStage.LOADING || _isLoading
        ?
    SpinKitCircle(color: Theme.of(context).primaryColor,size: 50,)
        :
    stage == ShiftsStage.DONE && shiftsProvider.upComingWeeks.isNotEmpty ?
    Column(
      children: [
        Expanded(
          child: SmartRefresher(
            ...
            child: ListView.builder(
              ...
              itemCount: shiftsProvider.upComingWeeks.length+1, // count here is listLength +1 as the last element is the SizedBox
              itemBuilder: (context, i)=> i == shiftsProvider.upComingWeeks.length ? const SizedBox(height: 75.0,):
              expansionTileCard(
                  context: context,
                  color: const Color(0xFFF6F6F6),
                  title: "${formatStringTimeToDayAndMonth(stringTime: shiftsProvider.upComingWeeks[i].week_start.toString())}  -  ${formatStringTimeToDayAndMonth(stringTime: shiftsProvider.upComingWeeks[i].week_end.toString())}",
                  doExpansion: (_){
                  ...
                  content: [
                    ListView.builder(
                      ...
                      shrinkWrap: true,
                      itemCount: shiftsProvider.upComingWeeks[i].offers.length,
                      itemBuilder: (context, index) =>
                          ClipRRect(
                              borderRadius: index == shiftsProvider.upComingWeeks[i].offers.length-1
                                  ?
                              ...
                              child: Container(
                                ...
                                child: Column(
                                  children: [
                                    GestureDetector(
                                        onTap: (){
                                          Provider.of<ShiftsProvider>(context,listen: false).setCurrentOffer(shiftsProvider.upComingWeeks[i].offers[index]);
                                          Navigator.pushNamed(context,
                                            ShiftDetails.routeName,
                                            arguments: {
                                              "offer" : shiftsProvider.upComingWeeks[i].offers[index]
                                            });
                                        },
                                        child:
                                        shiftCard(
                                    ...
                                    index == shiftsProvider.upComingWeeks[i].offers.length-1 ? const SizedBox() : const Divider()
                                  ],
                                ),
                              )),
                    ),
        ...
        :
    stage == ShiftsStage.DONE && shiftsProvider.upComingWeeks.isEmpty
        ?
    emptyShifts(context,media,'upCompingShifts')
        :
