...
class MessagingScreen extends StatefulWidget {
  ...
  Map passedData = {};
  MessagingScreen({this.passedData});

  @override
  _MessagingScreenState createState() => _MessagingScreenState(passedData: this.passedData);
}

class _MessagingScreenState extends State<MessagingScreen> with SingleTickerProviderStateMixin, AutomaticKeepAliveClientMixin {
  Map passedData = {};
  _MessagingScreenState({this.passedData});
  ...
  Future<bool> _checkPermission() async {
    ...
    if (io.Platform.isAndroid && androidInfo.version.sdkInt <= 28) {
...
//method to load image and update `uploadedImage`
  ...
  Future<void> getMultipleImageInfo() async {
    var mediaData = await ImagePickerWeb.getImageInfo;
    mimeType = mime(p.basename(mediaData.fileName));
  ...
  convertHtmlFileToUint8List(html.File file) async {
    final reader = html.FileReader();
    reader.readAsArrayBuffer(file);

    return await reader.onLoad.first;
  }

  Future<void> getMultipleFileInfo() async {
    try{
      print('--------kkkkkkkkk ');
      FilePickerResult result = await FilePicker.platform.pickFiles();
      print('sanya wahdaaaaaa000000 ');

      if(result != null) {
        print('resultresultresultresult ${result.files[0].size}');

        if (result.files[0].size > 20000000) {
          print('showToastshowToastshowToast');
          ...
          print('sanya wahdaaaaaa1111 ');
          if (mounted) {
            setState(() {
              docFile = result.files[0];
              // io.File(result.files.single.path);
              docFileName = result.names[0];
              print('sanya wahdaaaaaa2222 $docFileName');
            });
          }
  ...
  Future<void> getHistory() async {
    print('from getHistoryyyyyyyyyyyyyyy');
    // _chatMessages.clear();
    // if(history.messages == null){
    
    // Provider.of<ChatProvider>(context, listen: false).resetChatChannelHistory();
    //   And could we set up weekly data for Basingstoke from Monday pls?
    if (mounted) {
      setState(() {
        _isloadingMsgs = true;
      });
    }
    Provider.of<ChatProvider>(context, listen: false).loadMoreChatChannelHistory().then((_){
      toAndFromMessages();
      if (mounted)
        setState(() {
    ...
    // scrollToTop();
    // var logger = Logger();
    // logger.d("${Provider.of<ChatProvider>(context, listen: false).chatChannelHistory.messages.last.contents}");
  }
    ...
    msg.text = "$imageText";
      ...
      if(_imageFile != null){
        Provider.of<ChatProvider>(context,listen: false).chatAttachment(context,
            channelId: channel.id, filePath: kIsWeb ? "temp/${DateTime.now()}-$imgName" : _imageFile.path,
            fileName: imgName, imageBytes: kIsWeb ? imageBytes : null).then((_){
  ...
  void didChangeDependencies() {
    ...
    if(_isInit){
      ...
      //  channel = null;
      // _messages = [];
      // Provider.of<ChatProvider>(context, listen: false).chatChannelHistory = null;
      // channelName = null;
      // msgsToMe = [];
      // msgsFromMe = [];
      // systemMsgs = [];
      ...
      // // PickedFile pickedFile;
      // imageText = "Image";
      // sendingMessageStateResult = null ;
      // lastMsgPayloadId = null;
      // _isloadingMsgs = false;
      // waitToScrollInMillieSec = 1100;
      ...
      if(mounted){
        setState(() {
          ...
          // Provider.of<ChatProvider>(context, listen: false).setChatChannelHistory(passedData["conversation_messages"]);
          _messages = Provider.of<ChatProvider>(context, listen: false).chatChannelHistory == null ? [] : Provider.of<ChatProvider>(context, listen: false).chatChannelHistory.messages;

          if(passedData["conversation_messages"] == null && mounted){
            setState(() {
              channelName = passedData["channel_name"];
              Provider.of<ChatProvider>(context, listen: false).setChatChannelHistory(channelDetails.pubnub.channel('$channelName').history(chunkSize: 60));
              // Provider.of<ChatProvider>(context, listen: false).chatChannelHistory = channelDetails.pubnub.channel('$channelName').history(chunkSize: 60);
              _messages = Provider.of<ChatProvider>(context, listen: false).chatChannelHistory.messages;
            });
          }
      ...
      //     });
      //   }
      //   // if(io.Platform.isIOS && passedData["sender_name"] != null){
  ...
  Widget _previewImage({width, height}) {
    ...
    if (_imageFile != null /*|| docFile != null*/) {
      if (kIsWeb) {
        ...
        return _imageFile == null ? const SizedBox() : Container(
          width: width,
          height: height,
    ...
    return WillPopScope(
      ...
      child: GestureDetector(
        onHorizontalDragEnd: (DragEndDetails details){
          if (io.Platform.isIOS && _viewChatImage == null) {
            ...
            Scaffold(
                ...
                appBar: screenAppBar(context, media, centerTitle: false,
                  appBarHeight: 80,
                    ...
                    hideProfilePic: true,
                    showLeadingPop: kIsWeb ? false : true),
                body:
                  ...
                  children: [
                    Positioned(
                      ...
                      DashChat(
                                              ...
                                              Flexible(
                                                child: Container(
                                                    ...
                                                    child: Padding(
                                                          ...
                                                          InkWell(
                                                            onTap:(){
                                                              if (!kIsWeb) {
                                                                ...
                                                                openInANewTab(msg.customProperties["document_url"]);
                                                              }
                                                            },
                                                            child: Padding(
                                                      ...
                                                      InkWell(
                                                        onTap: kIsWeb ? () async{
                                                          if (await canLaunch(msg.customProperties['url'])) {
                                                            await launch(msg.customProperties['url'],
                                                              forceSafariVC: true,
                                                              forceWebView: true,
                                                          ...
                                                          children: [
                                                            Row(
                                                              ...
                                                              children: [
                                                                ...
                                                                SizedBox(
                                                                    height: kIsWeb ? 165 : 100,
                                                                    width: kIsWeb ? media.width *0.38 :  160,
                                  ...
                                  showAttachImgBottomSheet(context: context,
                                      ...
                                      onDocument: ()async{
                                        ...
                                        try{
                                          FilePickerResult result = await FilePicker.platform.pickFiles();
                                          if(result != null) {
                                            if (await io.File(result.files.single.path).length() > 20000000) {
                                              showToast("File size must be less than 20MB",icon: const Icon(Icons.clear,color: Colors.white,));
                                            }
                                            else{
                                              setState(() {
                                                docFile = io.File(result.files.single.path);
                                                docFileName = result.names[0];
                                              });
                                              debugPrint("hey this is the file name ************ ${await docFile.length()}");
                                            }
                                      ...
                                      onGalleryImg: kIsWeb ? (){
                                        // _pickImage();
                                        print('kokokokoklbhbhhbhbbhhbbhbhhbbhbhhbhbhb');
                                      }
                                      :
                                          ()async{
                                        print("onGalleryImg");
                        ...
                        scrollToBottom: kIsWeb ? false : true,
